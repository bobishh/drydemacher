...
pub async fn add_manual_version(
    ...
    design_service::add_manual_version(
        design_service::AddManualVersionRequest {
            ...
            geometry_backend: input.geometry_backend,
...
"#;
    ...
    #[test]
    fn parse_macro_params_supports_ecky_ir_controls() {
        let parsed = parse_macro_params(
            r#"(model
                (params
                  (number width 120 :min 20 :max 300)
                  (select profile "bulb" :options (("Bulb" "bulb") ("Lantern" "lantern")))
                  (toggle vents #t)
                  (image litho ""))
                (part body (cylinder 20 80 48)))"#
                .to_string(),
        );
        ...
        assert_eq!(
            parsed.params.get("profile"),
            Some(&ParamValue::String("bulb".to_string()))
        );
