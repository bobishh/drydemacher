...
pub async fn resolve_agent_prompt(
    ...
    #[cfg(unix)]
    {
        let pgid = state.prompt_pgids.lock().unwrap().remove(&request_id);
...
pub async fn wake_auto_agent(label: String, state: State<'_, AppState>) -> AppResult<()> {
    // If agent is already alive, do nothing.
    if state.auto_agent_pids.lock().unwrap().contains_key(&label) {
        return Ok(());
    }
    let notify = state.agent_wake.lock().unwrap().get(&label).cloned();
