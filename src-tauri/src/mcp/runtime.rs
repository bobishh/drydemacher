...
fn write_agent_instructions(
    ...
    let agents_md = format!(
        ...
        3. Call `request_user_prompt` with a SHORT friendly greeting only \
           ...
           plus `threadId` / `threadTitle` when the session is bound to a thread. \
        ...
        4. When the user sends the first queued message:\n\
           ...
           h. If a step will take more than a few seconds, call `session_activity_set`, and call \
              `session_activity_clear` when that step finishes.\n\
           i. Act on the request using `macro_replace_and_render` or `params_patch_and_render`.\n\
...
fn build_initial_prompt(agent: &AutoAgent, endpoint_url: &str) -> String {
    format!(
        ...
        plus `threadId` / `threadTitle` for the active thread. \
    ...
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        ...
        handle.block_on(async move {
            ...
            if let Some(session) = sessions.get_mut(&session_id) {
                ...
                session.status_text = Some(status_text);
                session.busy = false;
