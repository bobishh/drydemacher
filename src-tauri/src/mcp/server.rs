...
fn workflow_guide_text(state: &AppState) -> String {
    format!(
        concat!(
            ...
            "2. Read ecky://guides/cad-sdk when you need the actual helper surface instead of guessing SDK functions.\n",
            "3. Call workspace_overview.\n",
            "4. Call target_meta_get.\n",
            "5. For macro geometry/orientation questions, call target_macro_get.\n",
            "6. For exact chunks, call target_detail_get(section=...).\n",
            "7. Call semantic_manifest_get only when semantic bindings matter.\n",
            "8. Use target_get only when you truly need the full payload.\n",
            "9. Then mutate with params_patch_and_render, macro_replace_and_render, or semantic tools.\n",
            "10. Use measurement_annotation tools to encode what dimensions mean instead of leaving that meaning only in prose.\n",
            "11. For long steps, call long_action_notice and long_action_clear so Ecky can show clean busy status without scraping terminal text.\n"
        ),
...
fn tool_definitions() -> Vec<Value> {
    vec![
        ...
        json!({
            ...
            "description": "Request text input from the human in the Ecky UI, with optional image/CAD attachments from the same prompt panel. Blocks until the user submits or the timeout expires. The response includes promptText/attachments and, when the session is bound, threadId/threadTitle for the active thread context. Use this to drive the conversation loop: ask what they want, wait for their reply, then act on it. In active MCP mode, call this again immediately after each completed user-facing turn so Ecky can queue the next message.",
            "inputSchema": {
                "type": "object",
                "properties": {
