<script lang="ts">
  ...
  const dialogueState = $derived.by<DialogueState>(() => {
    ...
    if (isMcpMode) return { mode: 'mcp-idle' };
  ...
  // Lithophane post-processing
  let showLithophane = $state(false);
  let lithoImagePath = $state('');
  let lithoProjection = $state<'planar' | 'cylindrical' | 'spherical'>('planar');
  let lithoDepth = $state(3.0);
  let lithoInvert = $state(false);
  let lithoApplying = $state(false);
  ...
  let agentWakingMessage = $state<string | null>(null);
  let agentWakeError = $state<string | null>(null);
  let agentWakeTimer: ReturnType<typeof setTimeout> | null = null;
  let agentProcessing = $state(false);

  const WAKING_MESSAGES = [
    'let me grab my calipers',
    'let me fire up the lathe',
    'let me find my slide rule',
    'let me boot the CNC',
    'let me sharpen my pencil',
    'let me grab my micrometer',
    'let me warm up the mill',
    'let me find my feeler gauge',
    'let me dust off the bandsaw',
    'let me grab my T-square',
  ];
  ...
  $effect(() => {
    ...
    void resolveAgentPrompt(requestId, first.text).catch(() => { agentProcessing = false; });
  });

  $effect(() => {
    const count = activeAgentSessions.length;
    if (count > 0 && prevSessionCount === 0) {
      genieWakeUpCount++;
      isWaking = true;
      setTimeout(() => { isWaking = false; }, 1400);
    }
    prevSessionCount = count;
  });

  // Wake animation fires when agent first sends a prompt (pendingAgentPrompt appears).
  // No startup pre-waking — genie should be idle until agent actually greets.
  ...
  async function handleDialogueSubmit(prompt: string, attachments: Attachment[]) {
    switch (dialogueState.mode) {
      ...
      case 'mcp-idle': {
        ...
        // Wake dead auto-agents on demand.
        const agents = ($config.mcp?.autoAgents ?? []).filter(a => a.enabled && !a.startOnDemand);
        if (agents.length > 0) {
          agentWakingMessage = WAKING_MESSAGES[Math.floor(Math.random() * WAKING_MESSAGES.length)];
          agentWakeError = null;
          isWaking = true;
          genieWakeUpCount++;
          if (agentWakeTimer) clearTimeout(agentWakeTimer);
          agentWakeTimer = setTimeout(() => {
            if (agentWakingMessage) {
              agentWakingMessage = null;
              isWaking = false;
              agentWakeError = 'agent failed to connect — check logs';
            }
          }, 60_000);
          for (const a of agents) {
            void wakeAutoAgent(a.label).catch(() => {});
          }
        }
        break;
      }
  ...
  onMount(() => {
    ...
    const unlistenHistory = listen('history-updated', async () => {
      const prevVersionId = get(activeVersionId);
      await refreshHistory();
      // Auto-navigate to new agent-committed version when in MCP mode
      const tid = get(activeThreadId);
      if (!tid || !activeAgentSessions.length) return;
      const thread = get(history).find(t => t.id === tid);
      if (!thread?.messages) return;
      const versions = thread.messages.filter(
        (m): m is Message =>
          m.role === 'assistant' &&
          !!(m.output || m.artifactBundle) &&
          m.status !== 'error' &&
          !m.deletedAt,
      );
      if (!versions.length) return;
      const latest = versions[versions.length - 1];
      if (latest.id !== prevVersionId && latest.agentOrigin) {
  ...
  const genieMode = $derived.by(() => {
    ...
    if (agentProcessing || agentWakingMessage) return 'light';
    ...
    if (activeAgentSessions.length > 0) return isWaking ? 'waking' : 'light';
    return 'idle';
  });
  ...
  const genieBubble = $derived.by(() => {
    ...
    if (pendingAgentPrompt) {
      raw = pendingAgentPrompt.message || `${pendingAgentPrompt.agentLabel} is waiting for your input`;
    } else if (agentWakeError) {
      raw = agentWakeError;
    } else if (agentWakingMessage && isWaking) {
      raw = agentWakingMessage;
    } else {
      const atPhase = activeThreadHighestPhase;
      const threadError =
        atPhase === 'error'
  ...
  $effect(() => {
    if (pendingAgentPrompt?.requestId) {
      dismissedBubbleText = '';
      agentWakingMessage = null;
      agentWakeError = null;
      agentProcessing = false;
      isWaking = false;
      if (agentWakeTimer) { clearTimeout(agentWakeTimer); agentWakeTimer = null; }
      // Wake animation on first greeting
  ...
  async function pickLithoImage() {
    const file = await open({ multiple: false, filters: [{ name: 'Images', extensions: ['png', 'jpg', 'jpeg', 'webp'] }] });
    if (typeof file === 'string') lithoImagePath = file;
    else if (file && (file as { path?: string }).path) lithoImagePath = (file as { path: string }).path;
  }

  async function applyLithophane() {
    if (!lithoImagePath || !$workingCopy.macroCode) return;
    lithoApplying = true;
    try {
      const params = { ...$workingCopy.params, '__litho_image': lithoImagePath };
      const postProcessing: PostProcessingSpec = {
        displacement: { imageParam: '__litho_image', projection: lithoProjection, depthMm: lithoDepth, invert: lithoInvert }
      };
      const bundle = await renderModel($workingCopy.macroCode, params, postProcessing);
      session.setModelRuntime(bundle, null);
      session.setStlUrl(toAssetUrl(bundle.previewStlPath));
      showLithophane = false;
    } catch (e: unknown) {
      session.setError(`Lithophane Error: ${formatBackendError(e)}`);
    } finally {
      lithoApplying = false;
    }
  }
          ...
          <main class="viewport-area" role="presentation" class:onboarding-highlight={$onboarding.target === 'viewport'}>
            ...
            <div class="genie-layer" class:onboarding-active={$onboarding.isActive}>
              <VertexGenie 
                ...
                agentConnected={activeAgentSessions.length > 0 || isWaking || !!pendingAgentPrompt || agentProcessing}
              />
            </div>
            ...
            {#if $activeThreadId && ($workingCopy.macroCode || stlUrl)}
              <div class="viewport-overlay">
                ...
                <div class="export-actions">
                  ...
                  <button class="btn btn-xs btn-secondary" onclick={() => { showLithophane = !showLithophane; }} disabled={!stlUrl || showViewerBusyMask} title="Apply image displacement (lithophane)">◻ LITHO</button>
                ...
                {#if showLithophane}
                  <div class="litho-panel">
                    <div class="litho-row">
                      <button class="btn btn-xs" onclick={pickLithoImage}>
                        {lithoImagePath ? lithoImagePath.split(/[/\\]/).pop() : 'Select Image...'}
                      </button>
                      <select class="input-mono litho-select" bind:value={lithoProjection}>
                        <option value="planar">Planar</option>
                        <option value="cylindrical">Cylindrical</option>
                        <option value="spherical">Spherical</option>
                      </select>
                      <label class="litho-label">Depth (mm)</label>
                      <input class="input-mono litho-depth" type="number" min="0.1" max="20" step="0.5" bind:value={lithoDepth} />
                      <label class="litho-invert">
                        <input type="checkbox" bind:checked={lithoInvert} /> Invert
                      </label>
                      <button class="btn btn-xs btn-primary" onclick={applyLithophane} disabled={!lithoImagePath || lithoApplying}>
                        {lithoApplying ? 'Rendering...' : 'Apply'}
                      </button>
                    </div>
                  </div>
                {/if}
...
<style>
  ...
  .litho-panel { width: 100%; border-top: 1px solid var(--bg-300); padding-top: 8px; }
  .litho-row { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; }
  .litho-select { padding: 2px 4px; font-size: 0.7rem; background: var(--bg-200); border: 1px solid var(--bg-300); color: var(--text); cursor: pointer; }
  .litho-label { font-size: 0.68rem; color: var(--text-dim); white-space: nowrap; }
  .litho-depth { width: 60px; padding: 2px 4px; font-size: 0.7rem; }
  .litho-invert { display: flex; align-items: center; gap: 4px; font-size: 0.68rem; color: var(--text-dim); cursor: pointer; }
