<script lang="ts">
  ...
  $effect(() => {
    ...
    void resolveAgentPrompt({
      ...
      messageId: nextDelivery.delivery.id,
  ...
  async function answerAgentPrompt(requestId: string, promptText: string, attachments: Attachment[]) {
    pendingAgentPrompt = null;
    try {
      const preparedAttachments = await preparePromptAttachments(attachments);
  ...
  async function handleDialogueSubmit(prompt: string, attachments: Attachment[]) {
    switch (dialogueState.mode) {
      ...
      case 'mcp-idle': {
        ...
        try {
          preparedAttachments = await preparePromptAttachments(attachments);
        } catch (e) {
        ...
        try {
          queuedMessage = await queueAgentPrompt({
            threadId: currentVisibleTargetRef()?.threadId ?? $activeThreadId ?? null,
            promptText: prompt,
            attachments: preparedAttachments,
          });
        } catch (e) {
  ...
  const hasQueuedAgentMessageWithoutPrompt = $derived.by<boolean>(() => {
    ...
    return queuedUserMessages.some((message) => message.status === 'queued');
  });
