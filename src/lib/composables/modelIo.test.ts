...
test('Given a STEP artifact When STEP export runs Then it saves and exports that artifact', async () => {
  ...
  await io.exportModel('step', { modelId: 'm', modelStlPath: '', fcstdPath: '', manifestPath: '', contentHash: '', sourceKind: 'generated', exportArtifacts: [{ format: 'step', path: '/tmp/model.step', label: 'STEP', role: 'primary' }] }, { threeMf: '', multipartStlZip: '', stl: '', step: 'model.step', fcstd: '' }, [], false, 'model');
