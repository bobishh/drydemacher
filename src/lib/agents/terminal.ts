...
export function agentTerminalSessionKey(
  ...
  if (!snapshot) return '';
  return `${snapshot.agentId}:${snapshot.sessionNonce}`;
}
...
export function mergeAgentTerminalSnapshot(
  ...
  const sameSession =
    ...
    previous.sessionNonce === incoming.sessionNonce;
...
export function pickVisibleAgentTerminal(
  ...
  const anyAttention = [...active]
    .filter((snapshot) => snapshot.attentionRequired)
    .sort(byNewest)[0];
  if (anyAttention) return anyAttention;

  if (primaryAgentId && active.length) {
    const primary = active.find((snapshot) => snapshot.agentId === primaryAgentId) ?? null;
    if (primary) return primary;
  }

  const newestActive = [...active].sort(byNewest)[0] ?? null;
  if (newestActive) return newestActive;

  if (primaryAgentId) {
    const primaryFallback = inactive.find((snapshot) => snapshot.agentId === primaryAgentId) ?? null;
    if (primaryFallback) return primaryFallback;
  }

  return [...inactive].sort(byNewest)[0] ?? null;
}
...
export function pickAgentTerminalAttention(
  ...
  const visible = pickVisibleAgentTerminal(snapshots, primaryAgentId);
