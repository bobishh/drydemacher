...
- Internal runtime bundle adapter writes manifest, model STL, primary STEP artifact, and face topology targets for direct OCCT exports while presenting public geometry backend as `mesh`; multi-part exports preserve part bindings in the manifest.
- EckyRust render dispatch has a hidden direct-OCCT fast path for `.ecky` Core IR when the bundled SDK is complete. It falls back to the mesh renderer on direct-OCCT blockers or unsupported surface.
