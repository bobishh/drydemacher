...
<app-data>/model-runtime/<kind>/<model-id>/
  preview.stl, model.FCStd, manifest.json, bundle.json
```
...
<source-root>/<thread-slug>/
  ...
  ecky-project.json              existing mirror manifest + digest
```
...
- Existing `ecky-project.json` schema v1 remains the only folder sidecar.
...
- Successful Ecky/agent commits refresh a clean bound file atomically.
- A settled external save validates, renders, and becomes a normal new version.
...
- Immediate mirror, external-file watcher, and conflict guard.
...
- External/agent file edits become versions; Ecky never clobbers pending edits.
- Existing global history and artifact paths remain valid.
